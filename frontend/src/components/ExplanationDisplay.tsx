import { useLanguage } from '../hooks/useLanguage';
import type { ScoreExplanation } from '../types/analysis';
import { ExplanationSection } from './ExplanationSection';
import './ExplanationDisplay.css';

interface ExplanationDisplayProps {
  explanation: ScoreExplanation;
}

export function ExplanationDisplay({ explanation }: ExplanationDisplayProps) {
  const { t, translateFreeform } = useLanguage();

  const translatedSummary = translateFreeform(explanation.summary);

  return (
    <div className="explanation-container">
      <div className="explanation-header">
        <h3 className="explanation-title">
          {t('scoreExplanation')}
        </h3>
      </div>

      <div className="explanation-summary">
        <p className="explanation-summary-text">{translatedSummary}</p>
      </div>

      <div className="explanation-grid">
        <ExplanationSection
          title={t('positivePoints')}
          icon={
            <svg className="explanation-icon" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
          }
          items={explanation.positives}
          sentiment="positive"
          itemSymbol="+"
          formatListItem={translateFreeform}
        />

        <ExplanationSection
          title={t('negativePoints')}
          icon={
            <svg className="explanation-icon" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
          }
          items={explanation.negatives}
          sentiment="negative"
          itemSymbol="-"
          noItemsLabel={t('noCriticalIssues')}
          formatListItem={translateFreeform}
        />
      </div>
    </div>
  );
}

ExplanationDisplay.displayName = 'ExplanationDisplay';
